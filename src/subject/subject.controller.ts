...
export class SubjectController {
  ...
  async getSubject(@Res() response, @Param('id') subjectId: string) {
    try {
      const existingSubject = await this.subjectService.getSubject(subjectId);
      return response.status(HttpStatus.OK).json({
  ...
  @Post('search')
  async searchSubjectsByName(@Body('name') name: string) {
    const subjects = await this.subjectService.findByName(name);
    return subjects;
  }
}
