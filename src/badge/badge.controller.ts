...
@Put('/:id')
async updateBadge(@Res() response,@Param('id') badgeId: string,
@Body() updateBadgetDto: UpdateBadgeDto) {
  try {
   const existingBadge = await this.badgeService.updateBadge(badgeId, updateBadgetDto);
...
try {
  ...
  return response.status(HttpStatus.OK).json({
  message: 'All badges data found successfully',badgeData,});
 } catch (err) {
  return response.status(err.status).json(err.response);
 }
}
...
async getBadge(@Res() response, @Param('id') badgeId: string) {
 try {
    const existingBadge = await
this.badgeService.getBadge(badgeId);
    return response.status(HttpStatus.OK).json({
    message: 'Badge found successfully',existingBadge,});
 } catch (err) {
   return response.status(err.status).json(err.response);
 }
}
